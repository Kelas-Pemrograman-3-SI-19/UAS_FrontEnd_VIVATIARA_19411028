    ...
    <q-page padding>
            ...
            <div class="row">
              ...
              <div class="col">
                <q-banner inline-actions class="text-blue-grey-14">
                  <div class="text-h6">Edit Trevel</div>
                  <div>Edit Data Trevel</div>
                </q-banner>
              </div>
            </div>
          </div>
        ...
        <q-card flat>
          <q-card-section class="row">
            ...
            >
              <q-input
                v-model="form.TGLkebangkatan"
                filled
                label="TGLkebangkatan"
                :rules="[ val => val && val.length > 0 || 'Mohon Isi TGLkebangkatan']"
              />
...
export default {
  ...
  methods: {
    getData () {
      this.$axios.get(`trevel/getbyid/${this.$route.params.id}`)
        .then(res => {
          if (res.data.sukses) {
            this.form = res.data.data
          } else {
            this.$router.push({ name: 'dataMTrevel' })
          }
        })
    },
    onSubmit () {
      ...
      this.$axios.put(`trevel/edit/${this.$route.params.id}`, formData)
        .then(res => {
          if (res.data.sukses) {
